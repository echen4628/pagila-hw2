 ...
 */
SELECT distinct title FROM inventory JOIN film USING (film_id) WHERE film_id NOT IN (SELECT film_id FROM inventory JOIN rental USING (inventory_id) JOIN customer using (customer_id) JOIN address USING (address_id) JOIN city USING (city_id) JOIN country USING (country_id) WHERE country = 'United States') ORDER BY title;
